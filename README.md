...
> ## ⚠️ 重要な但し書き
>
> これは気体の物理を可視化する **教育・研究用のデモ**であり、同時に
...
- **オンライン（リンクで誰でも）:** **https://gas-iop-simulator-nbcyuehnq2nlhu7hxynappy.streamlit.app/**
- **手元で動かす:**
  ...
  IOP・気泡体積・充填率のグラフがその場で更新されます。サイドバー上部の
  「言語 / Language」で日本語／英語を切り替えられます。

Python から直接呼ぶこともできます:
...
v1.2 の文献適合（vitrectomized 較正, 4 成分定数 Q + 球冠→球）:
...
4. **未実装.** 前房房水を含めた流体容積、個体別の眼剛性較正、圧–体積実測カーブの差し込み。
...
```
app.py                        Streamlit 対話 UI（v0 / v1.2 切替・日英切替）
